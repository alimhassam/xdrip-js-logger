  ...
  if [ -e $CALFILE ]; then
    ...
    then
      ...
      if [ $(bc <<< "($epochdate - $calDate) < 420") -eq 1 ]; then
        ...
        echo "calibrationJSON=$calibrationJSON"
      else
...
bt-device -r $id
echo "Calling xdrip-js ... node logger $transmitter $calibrationJSON"
# FIXME: $calibrationJSON contains quotes which need escaping i think
DEBUG=smp,transmitter,bluetooth-manager timeout 360s node logger $transmitter "${calibrationJSON}"
...
#  if test  `find ./last_sensor_change -mmin -180`
#  then
...
#  done
#
#  #bg0=$(jq ".[0].glucose" /root/myopenaps/monitor/glucose.json)
#  #bg1=$(jq ".[1].glucose" /root/myopenaps/monitor/glucose.json)
#  #bg2=$(jq ".[2].glucose" /root/myopenaps/monitor/glucose.json)
#
#
#
...
if [ -e "/usr/local/bin/fakemeter" ]; then
  ...
  if ! listen -t 4s >& /dev/null ; then 
    echo "Sending BG of $glucose to pump via meterid $meterid"
    fakemeter -m $meterid  $glucose 
  else
    echo "Timed out trying to send BG of $glucose to pump via meterid $meterid"
  fi
fi
